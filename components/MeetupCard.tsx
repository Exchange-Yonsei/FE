...
export function MeetupCard({ meetup }: { meetup: PublicMeetup }) {
  ...
  return (
    <article className="flex h-full flex-col rounded-3xl border border-blue-soft bg-white p-5 shadow-soft">
      <div className="flex items-start justify-between gap-3">
        <CategoryBadge category={meetup.category} />
        <MeetupStatusBadge status={meetup.status} isFull={isFull} />
      </div>
      <Link href={`/meetups/${meetup.id}`} className="mt-4 text-xl font-black leading-tight text-ink hover:text-yonsei-secondary">
        {meetup.title}
      </Link>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {["Korean Host", "Small Group", "English Friendly"].map((badge) => (
          <span key={badge} className="rounded-full border border-blue-soft bg-yonsei-light px-3 py-1 text-xs font-bold text-yonsei-secondary">
            {badge}
          </span>
        ))}
      </div>
      <div className="mt-5 space-y-2 text-sm text-stone-700">
        <p className="flex items-center gap-2">
          <MapPin className="h-4 w-4 text-yonsei-secondary" aria-hidden />
          {meetup.location}
        </p>
        <p className="flex items-center gap-2">
          <CalendarClock className="h-4 w-4 text-yonsei-secondary" aria-hidden />
          {formatDateTime(meetup.starts_at)}
        </p>
        <p className="flex items-center gap-2">
          <Users className="h-4 w-4 text-yonsei-secondary" aria-hidden />
      ...
      <div className="mt-auto flex items-center justify-between pt-5">
        ...
        <Link className="text-sm font-bold text-yonsei-secondary hover:text-yonsei-primary" href={`/meetups/${meetup.id}`}>
          View
        </Link>
      </div>
    </article>
